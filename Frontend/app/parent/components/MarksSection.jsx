...
const MarksSection = memo(({ classroom, currentStudentId }) => {
  const { t } = useTranslation();
  const { user } = useAuth();

  // Filter marks to only show the ones for the current student
  const filteredMarks = useMemo(() => {
    if (!classroom?.marks?.length) return [];
    return classroom.marks.filter(
      (mark) => mark.student?._id === currentStudentId
    );
  }, [classroom?.marks, currentStudentId]);

  // Sort the filtered marks by date
      ...
      datasets: [
        {
          data: sortedMarks.map((mark) => {
            const score = mark.marksObtained || mark.score || 0;
            const total = mark.totalMarks || mark.totalScore || 100;
  ...
  // If no marks are available for this student
    ...
    return (
      <View className="bg-white rounded-lg p-4" style={{ height: 300 }}>
        <Text className="text-gray-500 italic text-center">
          {t("No marks available for this student")}
        </Text>
        <LineChart
          data={lineChartData}
      ...
      {sortedMarks.map((mark, index) => (
        ...
        >
          <Text className="font-bold text-gray-800 mb-2">
            {t("Exam")} : {mark.exam || mark.assessment || t("Test")}
          </Text>

          <View className="flex-row flex-wrap">
            <View className="w-4/12 items-center justify-center">
              <ScoreGaugeComponent
                score={mark.marksObtained || mark.score || 0}
                total={mark.totalMarks || mark.totalScore || 100}
                size={90}
              />
            </View>
            ...
            <View className="w-8/12">
              ...
              <ComparisonChartComponent
                yourScore={mark.marksObtained || mark.score || 0}
          ...
          {mark.topic && (
            <Text className="text-gray-600 mt-2">{mark.topic}</Text>
          )}

          {mark.remarks && (
            <Text className="text-gray-600 italic mt-1">{mark.remarks}</Text>
          )}
